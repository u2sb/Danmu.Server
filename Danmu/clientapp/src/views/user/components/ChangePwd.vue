            ...
            return {
                ...
                rules: {
                    ...
                    newP: [
                        ...
                        { validator: (r, v, c) => v !== this.form.oldP ? c() : c('新密码不得与旧密码相同'), trigger: 'change' },
                        { min: 6, message: '密码长度不能低于6位', trigger: 'change' }
                    ],
                    confirmP: [{ validator: (r, v, c) => v === this.form.newP ? c() : c('两次密码不一致'), trigger: 'change' }]
                }
            }
        },
        computed: {
        ...
        methods: {
            ...
            submit() {
                ...
                this.$refs.form.validate(v => {
                    if (!v) return
                    this.loading = true
                    changePwd({ ...this.form, uid: this.id })
                        .then(() => elAlert('修改成功，请重新登陆', () => this.$store.dispatch('user/logout')))
                        .finally(() => {
                            this.loading = false
                            this.clearForm()
                        })
