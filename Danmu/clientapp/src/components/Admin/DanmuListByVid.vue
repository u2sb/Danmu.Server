...
export default {
  ...
  mounted: function() {
    this.showVidSelect = true
    this.GetVids()
    this.GetCount(this.vid)
    this.GetData(this.vid, this.pageSize, this.currentPage)
  },
  methods: {
    ...
    GetCount(vid) {
      this.$http
        .get('/api/admin/danmulist/countbyvid', {
          params: {
            vid: vid
          }
        })
    ...
    GetVids() {
      this.$http.get('/api/admin/danmulist/vids').then(res => {
        let dataObj = eval(res.data)
        if (dataObj.code === 0) {
          this.vids = dataObj.data
        } else if (dataObj.code === 401) {
          this.$router.push({ path: '/login', query: { url: this.$router.fullPath } })
        }
      })
    },
    GetData(vid, size, page) {
      this.loading = true
      this.$http
        .get('/api/admin/danmulist/querybyvid', {
          params: {
            vid: vid,
            size: size,
            page: page
